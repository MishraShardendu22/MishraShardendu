...
export const LinkGroup = ({ items, isMobile }: LinkGroupProps) => (
  <div className={`${isMobile ? 'space-y-2' : 'space-y-3'}`}>
    {items.map((item) => {
      ...
      const isExternal =
        item.external || (!isInternalAnchor && !isMailto && item.href.startsWith('http'))
      ...
      return (
        ...
        >
          <div
            className={`w-5 lg:w-6 h-5 lg:h-6 flex items-center justify-center group-hover:scale-110 ${motionSafe}`}
          >
            {item.icon}
          </div>
