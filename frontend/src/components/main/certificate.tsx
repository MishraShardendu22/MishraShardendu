...
import { Award, Clock, Download, ArrowRight, Star } from 'lucide-react'
        ...
        <div className="mx-auto mt-20 grid max-w-2xl grid-cols-1 gap-6 lg:mx-0 lg:max-w-none lg:grid-cols-2">
          {certifications.length === 0 ? (
            ...
            certifications.slice(0, 6).map((cert) => (
              <Card key={cert.inline?.id || cert.inline.id} className="group relative overflow-hidden hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500 border-border/50 hover:border-primary/30 bg-card/80 backdrop-blur-sm">
                ...
                <CardContent className="space-y-3 relative z-10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {cert.skills.slice(0, 4).map((skill, idx) => (
                      <Badge key={idx} variant="outline" className="text-xs bg-secondary/5 hover:bg-secondary/10 border-secondary/20 text-foreground transition-colors">
                        {skill}
                      </Badge>
                    ))}
        ...
        {certifications.length > 6 && (
          <div className="mt-20 text-center">
            <Link href="/certifications">
              <Button variant="outline" size="lg" className="group px-8 py-3 bg-card/80 backdrop-blur-sm border-primary/30 hover:border-primary/50 hover:bg-primary/10 transition-all duration-300">
                <span className="text-foreground font-medium">View All Certifications</span>
